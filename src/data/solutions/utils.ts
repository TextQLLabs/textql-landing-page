...
// Import all solution markdown files
const solutionModules = import.meta.glob('./*/solution.md', { eager: true, query: '?raw', import: 'default' }) as Record<string, string>;
 ...
 */
function parseFrontmatter(content: string): Solution | null {
  try {
    ...
    return frontmatter as Solution;
  } catch (error) {
...
/**
 * Load all solutions from the solution markdown files.
 */
export function loadSolutions(): Solution[] {
  const solutions = Object.entries(solutionModules).map(([path, content]) => {
    const solution = parseFrontmatter(content as string);
    if (!solution) {
      console.warn(`No frontmatter found in solution file: ${path}`);
      return null;
    }
    return solution;
